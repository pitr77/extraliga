...
export default async function handler(req, res) {
  try {
    ...
    // --- Log a odpoveď ---
    console.log(`✅ Počet zápasov: ${allMatches.length}`);
    res.status(200).json({
      matches: allMatches,
      teamRatings,
      playerRatings: {}, // hráčske štatistiky zatiaľ nie
    });
  } catch (err) {
    console.error("❌ Chyba pri fetchnutí NHL dát:", err);
